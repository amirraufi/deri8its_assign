...
# Greeks (spot‑denominated, undiscounted, same assumptions as pricing)
...
def bs_delta(S: float, K: float, T: float, sigma: float, is_call: bool) -> float:
    ...
    d1 = (math.log(S / K) + 0.5 * sigma * sigma * T) / (sigma * math.sqrt(T))
    N = lambda x: 0.5 * (1 + math.erf(x / math.sqrt(2)))
    return N(d1) if is_call else N(d1) - 1
...
# plotting the volatility smile (iv vs. ln(K/S) for understanding)
...
async def main(args) -> None:
    ...
    while time.time() < end_time:
        ...
        tick['fit_iv'] = tick['instrument'].map(iv_map)
        if not iv_map:
        ...
        # ------------------------------------------------------------------
# Greeks for every live instrument
# ------------------------------------------------------------------
        expiry_ts = time.mktime(time.strptime(EXPIRY, "%d%b%y"))
        T_snap = max(expiry_ts - time.time(), 0.0) / 31557600.0    # years

        def greek_row(r):
            iv_here = iv_map.get(r.instrument)
            if iv_here is None or np.isnan(iv_here):
                return pd.Series({'delta': np.nan, 'gamma': np.nan})
            delta = bs_delta(r.index_price, r.strike, T_snap, iv_here, r.type == 'C')
            gamma = bs_gamma(r.index_price, r.strike, T_snap, iv_here)
            return pd.Series({'delta': delta, 'gamma': gamma})

        tick[['delta', 'gamma']] = tick.apply(greek_row, axis=1)
        ...
        # Snapshot CSV output
        # ------------------------------------------------------------------
        cols_keep = ['instrument', 'strike','fit_iv', 'delta', 'gamma',
             'my_mark_px', 'mark_px', 'is_custom']
        df_out = (tick[cols_keep]
